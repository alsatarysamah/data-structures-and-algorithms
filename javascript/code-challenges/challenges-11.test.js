...
const totalSum = (input) => {
  ...
  let x=0;
  input.forEach(element =>{
    sum=element.reduce(sumation,0)
    x+=sum;
  })
  function sumation(sum,current)
  {
    return sum+current;
  }
  return x;
...
Write a function named findShortest that, given the Star Wars
 data from Challenge 6, uses any combination of filter, map and reduce to return the 
