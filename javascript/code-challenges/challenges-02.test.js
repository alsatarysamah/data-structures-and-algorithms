...
const forLoopTwoToThe = (arr) => {
 ...
 {
    if (arr[i]>=0)
   res.push( Math.pow(2,arr[i]))
    else res.push(1/ Math.pow(2,Math.abs(arr[i])))
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach(element => {
    if (element>=0)
    res.push( Math.pow(2,element))
     else res.push( 1/Math.pow(2,Math.abs(element)))
...
the same output as your forLoopTwoToThe function from challenge 4
  ...
  return arr.map(element =>
    {
      ...
      return Math.pow(2,element);
       else return( 1/Math.pow(2,Math.abs(element)))
